...
begin
  ...
  if to_regclass('public.game') is null then
    raise exception 'public.game table does not exist';
  end if;
  if to_regclass('public.participants') is null then
    raise exception 'public.participants table does not exist';
  end if;
end
$$;
...
alter table public.participants alter column client_id drop default;
alter table public.participants
...
set name = coalesce(name, title, '팀 편성'),
    group_count = coalesce(group_count, team_count::smallint, 0),
    group_size = coalesce(group_size, team_size::smallint, 0);
...
set client_id = coalesce(client_id, gen_random_uuid()),
    highest_average = coalesce(highest_average, 0),
...
alter table public.game alter column id drop identity if exists;
